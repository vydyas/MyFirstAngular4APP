...
export const DROPDOWN_CONTROL_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => SmartDropdown),
    multi: true
};
const noop = () => {
};
...
export class SmartDropdown implements OnInit, ControlValueAccessor {
    ...
    onItemClick(item: ListItem, index: number, evt: Event){
                ...
                let dataContains = this.contains(this.data, item);
                if(!dataContains) {
                  this.data.push(item)
                }
    ...
    contains(a, obj) {
      var i = a.length;
      while (i--) {
         if (a[i].itemName === obj.name || a[i].itemName === obj.itemName) {
             return true;
         }
      }
      return false;
...
@NgModule({
  ...
  declarations: [SmartDropdown, ListFilterPipe],
  exports:      [SmartDropdown]
})
